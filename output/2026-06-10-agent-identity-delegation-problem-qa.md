...
qa_date: 2026-06-10
qa_checks: em-dashes, negation-correction, ai-patterns
---
...
Least privilege is the right principle. It has been the right principle for thirty years and it has not gone away. The difficulty is the execution. Scoping an agent finely enough that it can read the specific files it needs from one location and pass them to one service, and nothing more, is genuinely hard to do with the tools most teams have. Intent-based, task-scoped authorization does not exist at the protocol level yet.
